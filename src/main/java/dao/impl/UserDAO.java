...
public class UserDAO{
    ...
    public boolean update(User user) throws DAOException {
        try (PreparedStatement ps = ConnBuilder.connection.prepareStatement("UPDATE users SET first_name=?, last_name+=?," +
                "phone_number=?, status=?, address_id=? WHERE id=?")) {
    ...
    public boolean insert(User user) throws DAOException {
        try (PreparedStatement preparedStatement = ConnBuilder.connection.prepareStatement("INSERT INTO users VALUES (NULL, ?, ?, ?)")){
    ...
    public void deleteById ( int id) throws DAOException{
        try (Statement stmt = ConnBuilder.connection.createStatement()){
            ResultSet resultSet = stmt.executeQuery("SELECT * FROM users u WHERE u.id =" + id);
            User user = null;
            if (resultSet.next())
            {
                user = fetch(resultSet);
